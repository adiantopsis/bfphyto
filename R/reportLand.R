...
reportLand <- function(lat,
  ...
  intersect <- st_intersects(ponto, st_transform(estados, crs = 4326), sparse = F)
  i <- as.logical(intersect)
  biom_i <- st_intersects(ponto, st_transform(biomas, crs = 4326), sparse = F)
  ib <- as.logical(biom_i)
  
  estado_local <- estados[i,]
  bioma_local <- biomas[ib, ]
